...
[AddComponentMenu("Camera-Control/Mouse Look")] public class MouseLookNeck : MonoBehaviour {
...
void Update ()
{
	...
	{
		rotationY += Input.GetAxis("Mouse Y") * sensitivityY;
			rotationY += gunScript.rotOffset.x;
		rotationY = ClampAngle (rotationY, minimumY, maximumY);
		
		Quaternion yQuaternion = Quaternion.AngleAxis (-rotationY, Vector3.right);
		transform.localRotation = originalRotation * yQuaternion;

		//Vector3 temp = gunScript.rotOffset + transform.localRotation.eulerAngles;
			//transform.localRotation.eulerAngles.Set(temp.x,temp.y,temp.z);
			print (transform.localRotation.eulerAngles.ToString());
	}
